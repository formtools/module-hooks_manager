...
function hm_get_rule($hook_id)
{
  ...
  $query = mysql_query("
    SELECT *
    FROM   {$g_table_prefix}module_hooks_manager_rules hmr, {$g_table_prefix}hooks h
    WHERE  h.hook_id = hmr.hook_id AND
           h.hook_id = $hook_id
      ");
...
function _hm_update_code_rule($current_hook_id, $info)
{
  ...
  ft_delete_hook($current_hook_id);
...
/**
 * This function retrieves and parses out all hook data from the /global/misc/hook_data.ini file provided by the core.
 *
...
function hm_get_hooks()
{
  ...
  $info = parse_ini_file("$g_root_dir/global/misc/hook_data.ini", true);
  
  $code_hooks     = array();
  $template_hooks = array();
  while (list($key, $value) = each($info))
  {
    if (strpos($key, "code_hook") !== false)
      $code_hooks[$value["name"] . ", " . $value["when"]] = $value;
    if (strpos($key, "template_hook") !== false)
      $template_hooks[$value["name"]] = $value;
  }
...
function hm_convert_hook_info_to_json($js_var_name, $hook_info)
{
  ...
  {
    $file = $hook_data["file"];
    $location = isset($hook_data["when"]) ? $hook_data["when"] : "";
    $name = $hook_data["name"] . "," . $location;

    $params = isset($hook_data["params"]) ? $hook_data["params"] : array();
    $escaped_params = array();
    foreach ($params as $param)
      $escaped_params[] = "\"$param\"";
    $escaped_params_str = join(", ", $escaped_params);
    ...
    $escaped_overridable_str = join(", ", $escaped_overridable);
